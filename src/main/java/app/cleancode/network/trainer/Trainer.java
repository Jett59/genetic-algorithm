...
public Trainer(NetworkDescription networkDescription, List<Input> dataset, TrainingScorer<Input> scorer, ActivationFunction activationFunction) {
...
public void train(double mutationRate, SplittableRandom rand) {
    ...
    for (ScoredNetwork network : networks) {
        newNetworks.add(scoreNetwork(network.network().mutate(mutationRate + rand.nextDouble() - 0.5, rand)));
...
private ScoredNetwork scoreNetwork(Network network) {
    ...
    for (Input input : dataset) {
        score += scorer.getScore(input, network.apply(input.getNetworkInputs(), activationFunction));
    }
    score /= dataset.size();
